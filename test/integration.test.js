...
test('ST → ST', () => {
  timezones.forEach((tz) => {
    ...
    assert.equal(str, 'Dec 5th, 5:00am', 'init-time')
    assert.equal(isDST, false, 'init-dst-off ' + tz)
    //move it to a new time in ST
    d1 = d1.add(3, 'days')
    str = d1.format('nice')
    isDST = d1.timezone().current.isDST
    assert.equal(str, 'Dec 8th, 5:00am', 'new-date')
    assert.equal(isDST, false, 'still-dst-off')
  })
})
...
test('ST → DST', () => {
  timezones.forEach((tz) => {
    ...
    let str = d1.format('nice')
    let isDST = d1.timezone().current.isDST
    assert.equal(str, 'Dec 5th, 10:00am', 'init-time')
    assert.equal(isDST, false, 'init-dst-off ' + tz)
    //move it to a new time in DST - April 5th
    d1 = d1.add(4, 'months')
    str = d1.format('nice')
    isDST = d1.timezone().current.isDST
    assert.equal(str, 'Apr 5th, 10:00am', 'new-date')
    assert.equal(isDST, true, 'dst-now-on')
  })
})
...
test('DST → DST', () => {
  timezones.forEach((tz) => {
    ...
    let str = d1.format('nice')
    let isDST = d1.timezone().current.isDST
    assert.equal(str, 'Jun 5th, 3:00pm', 'init-time')
    assert.equal(isDST, true, 'init-dst-on ' + tz)
    //move it to a new time in DST - July 5th
    d1 = d1.add(1, 'months')
    str = d1.format('nice')
    isDST = d1.timezone().current.isDST
    assert.equal(str, 'Jul 5th, 3:00pm', 'new-date')
    assert.equal(isDST, true, 'dst-still-on')
  })
})
...
test('DST → ST', () => {
  timezones.forEach((tz) => {
    ...
    let str = d1.format('nice')
    let isDST = d1.timezone().current.isDST
    assert.equal(str, 'Jun 5th, 8:00pm', 'init-time')
    assert.equal(isDST, true, 'init-dst-on ' + tz)
    //move it to a new time in ST - December 5th
    d1 = d1.add(6, 'months')
    str = d1.format('nice')
    isDST = d1.timezone().current.isDST
    assert.equal(str, 'Dec 5th, 8:00pm', 'new-date')
    assert.equal(isDST, false, 'dst-now-off')
  })
})
